...
for a in "$@" ; do
  case $a in
    --sessiondir=*)    sessionDir="${a#*=}"; shift ;;
    --fieldmapdir=*)    fieldmapDir="${a#*=}"; shift ;;
    --structdir=*)      structDir="${a#*=}"; shift ;;
    --transdir=*)       transDir="${a#*=}"; shift ;;
    --mrcatdir=*)       MRCATDIR="${a#*=}"; shift ;;
  esac
done

transDir=$sessionDir/struct_gre_transform
fieldmapImg="fg"
...
imcp $fieldmapDir/brainmask/${fieldmapImg}_restore_brain_mask $fieldmapDir/${fieldmapImg}_brain_mask_bet
...
fieldmapMask=$fieldmapDir/${fieldmapBase}_brain_mask_bet
...
case $flgReg in
  ...
  *)
    ...
    # LT 23/08/16 - added this line to improve registration for some tricky sessions
    fslmaths $structMask -s 0.5 -thr 0.1 -bin ${structMask}_tmp

    # 12 degrees-of-freedom registration to the structural
    echo "  12 dof linear registration"
    flirt -dof 12 -ref $structImg -refweight ${structMask}_tmp -in $fieldmapImg -inweight $fieldmapMask -omat $transDir/${fieldmapBase}_to_${structBase}.mat
    ;;
esac
...
applywarp --rel --interp=nn -i ${structMask}_tmp -r $fieldmapImg --premat=$transDir/${structBase}_to_${fieldmapBase}.mat -o $fieldmapDir/${structBase}_brain_mask_lin
...
fnirt --ref=$structImg --refmask=${structMask}_tmp --in=$fieldmapImg --aff=$transDir/${fieldmapBase}_to_${structBase}.mat --cout=$transDir/${fieldmapBase}_to_${structBase}_warpcoef --config=$config
...
applywarp --rel --interp=nn -i ${structMask}_tmp -r $fieldmapImg -w $transDir/${structBase}_to_${fieldmapBase}_warpfield -o $fieldmapDir/${fieldmapBase}_brain_mask
# and bring the actualy structural image along, mostly for inspection purposes
